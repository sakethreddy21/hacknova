...
const  ProductCard=()=> {
  ...
  const bookAppointment = async (name:string, date: string, time: string) => {
    const updatedProducts = await appointmentupdater(name, 'patient1', date, time);
...
<>
      ...
      {products.map((product) => (
        <Card key={product.id} className="w-full sm:w-[44%] md:w-1/3 lg:w-1/4 shadow-lg p-6 mb-6 bg-white rounded-3xl sm:p-6 lg:p-8">
          <CardHeader>
            <Image
      ...
      <Button className="bg-blue-400" variant="outline">Book appointment</Button>

      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className='flex justify-center mb-4'>Book the appointment</DialogTitle>
      ...
      <PopoverContent className="w-auto p-0" align="start">
        <Calendar
          mode="single"
          selected={date}
          onSelect={(value: Date | undefined) => setDate(value || new Date())}
          initialFocus
        />
      </PopoverContent>
    </Popover>
      ...
      <input
        ...
        <DialogFooter>
          <DialogClose asChild>
          <Button type="submit" onClick={() => bookAppointment(product.name, date.toISOString(), time)}>Save changes
